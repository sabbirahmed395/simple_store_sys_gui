<template>
  <div>
    <h2 class="intro-y text-lg font-medium mt-10">Item List</h2>
        ...
        <table class="table table-report -mt-2">
          <thead>
            <tr>
              ...
              <th class="whitespace-nowrap">ITEM DESCRIPTION</th>
              <th v-if="isStoreExecutive" class="whitespace-nowrap">STOCK</th>
              <th class="whitespace-nowrap">STATUS</th>
              <th class="text-center whitespace-nowrap" v-if="isAdmin">ACTIONS</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(item, k) in items" :key="k" class="intro-x">
              <td class="font-medium whitespace-nowrap">{{ item.name }}</td>
              <td class="font-medium whitespace-nowrap">{{ item.description }}</td>
              <td v-if="isStoreExecutive" class="font-medium whitespace-nowrap">{{ item.stock }}</td>
              <td class="font-medium whitespace-nowrap">{{ item.status }}</td>
...
const getItems = async () => {
  try {
    let res = await http.get("items", { params: { page: page.value } });
    items.value = res.data.data;
    meta.value = res.data.meta;
    pageCount.value = meta.value.last_page;
    listComment.value = `Showing ${meta.value.from} to ${meta.value.to} of ${meta.value.total} Items`;
  } catch (err) {
    console.log(err);
  }
};

const actionDelete = async (item_id) => {
  let deleteConsent = confirm("Are you sure you want to delete the Item?");
  if (deleteConsent) {
    try {
      await http.delete(`items/${item_id}`).then((response) => {
